...
def test_datasets_are_well_formed():
    for c in datasets.INJECTION_CASES:
        assert c["forbidden_substrings"] and c["notes"]
    for c in datasets.GROUNDING_CASES + datasets.CITATION_CASES:
        assert c["policy_context"]
